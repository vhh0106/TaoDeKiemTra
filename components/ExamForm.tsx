                    ...
                    <li key={step} className={`relative ${stepIdx !== steps.length - 1 ? 'pr-8 sm:pr-20' : ''} flex-1`}>
                        ...
                        >
                            {currentStep > step ? (
                             ...
                             <span className="absolute top-10 text-center text-xs sm:text-sm font-medium text-slate-600">{stepLabels[stepIdx]}</span>
                        </button>
                    </li>
                ))}
            </ol>
...
const ExamForm: React.FC<ExamFormProps> = ({ formData, setFormData, onSubmit, isLoading }) => {
    const isSpecialSubject = useMemo(() => ['Ngoại ngữ 1 (Tiếng Anh)', 'Ngữ văn'].includes(formData.subject), [formData.subject]);
    ...
    const { totalPercentage, totalScore } = useMemo(() => {
        ...
        const totalPercentage = questionTypes.reduce((sum, item) => sum + item.percentage, 0);
    ...
    return (
        <form className="space-y-8 bg-white p-6 sm:p-8 rounded-2xl shadow-xl border border-slate-200/80">
            ...
            <div className="min-h-[350px] pt-4">
                {step === 1 && (
                    <Section title="1. Thông tin chung" description="Thiết lập các thông tin cơ bản cho đề kiểm tra.">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                            <div>
                                <label htmlFor="schoolLevel" className="block text-sm font-medium text-slate-700 mb-1">Cấp học</label>
                                <select id="schoolLevel" value={formData.schoolLevel} onChange={e => handleChange('schoolLevel', e.target.value)} className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                             ...
                             <div>
                                ...
                                <select id="grade" value={formData.grade} onChange={e => handleChange('grade', e.target.value)} className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                                    {grades.map(grade => <option key={grade} value={grade}>{grade}</option>)}
                                </select>
                            </div>
                             <div>
                                <label htmlFor="subject" className="block text-sm font-medium text-slate-700 mb-1">Môn học</label>
                                <select id="subject" value={formData.subject} onChange={e => handleChange('subject', e.target.value)} className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                                    {subjects.map(subject => <option key={subject} value={subject}>{subject}</option>)}
                                </select>
                            </div>
                            <div>
                                <label htmlFor="textbook" className="block text-sm font-medium text-slate-700 mb-1">Bộ sách</label>
                                <select id="textbook" value={formData.textbook} onChange={e => handleChange('textbook', e.target.value)} className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm">
                             ...
                             <div>
                                ...
                                <input id="duration" type="number" value={formData.duration} onChange={e => handleChange('duration', parseInt(e.target.value, 10))} className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm" />
                            </div>
                        </div>
                    </Section>
                )}
                 ...
                 {step === 2 && !isSpecialSubject && (
                    <Section title="2. Phân bổ câu hỏi và điểm" description="Điều chỉnh số lượng câu hỏi, tỉ lệ % và điểm số cho từng dạng. Tổng điểm phải là 10.">
                        <div className="space-y-3">
                            ...
                            <QuestionTypeInput label="Trắc nghiệm" value={formData.multipleChoice} onChange={(f, v) => handleQuestionTypeChange('multipleChoice', f, v)} />
                            <QuestionTypeInput label="Đúng/Sai" value={formData.trueFalse} onChange={(f, v) => handleQuestionTypeChange('trueFalse', f, v)} />
                            <QuestionTypeInput label="Trả lời ngắn" value={formData.shortAnswer} onChange={(f, v) => handleQuestionTypeChange('shortAnswer', f, v)} />
                            <QuestionTypeInput label="Tự luận" value={formData.essay} onChange={(f, v) => handleQuestionTypeChange('essay', f, v)} />
                        </div>
                        <div className="mt-4 pt-4 border-t border-dashed flex flex-col sm:flex-row justify-end items-center gap-4 text-base font-semibold">
                            <div className={`flex items-center gap-2 p-2 rounded-md ${totalPercentageError ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100'}`}>
                                <span>Tổng tỉ lệ: {totalPercentage}%</span>
                                {totalPercentageError && <span className="text-xs">(Phải là 100%)</span>}
                            </div>
                            <div className={`flex items-center gap-2 p-2 rounded-md ${totalScoreError ? 'text-red-600 bg-red-100' : 'text-green-600 bg-green-100'}`}>
                ...
                {( (step === 3 && !isSpecialSubject) || (step === 2 && isSpecialSubject) ) && (
                     <Section title={isSpecialSubject ? "2. Nội dung và Yêu cầu" : "3. Nội dung và Yêu cầu"} description="Cung cấp nội dung kiến thức và các yêu cầu đặc biệt để AI tùy chỉnh đề bài tốt hơn.">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-grow">
                             <div className="flex flex-col">
                                <label htmlFor="knowledgeContent" className="block text-sm font-medium text-slate-700 mb-2">Nội dung kiến thức</label>
                                <textarea id="knowledgeContent" value={formData.knowledgeContent} onChange={e => handleChange('knowledgeContent', e.target.value)} placeholder="VD:&#10;- Bài 1: Sự đa dạng của thế giới sống&#10;- Bài 2: Các giới sinh vật&#10;- Chủ đề: Quang hợp và hô hấp" className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm flex-grow min-h-[200px]" />
                            </div>
                            <div className="flex flex-col">
                                <label htmlFor="additionalRequirements" className="block text-sm font-medium text-slate-700 mb-2">Yêu cầu bổ sung (tùy chọn)</label>
                                 <textarea id="additionalRequirements" value={formData.additionalRequirements} onChange={e => handleChange('additionalRequirements', e.target.value)} placeholder="VD:&#10;- Cần có 1 câu hỏi trắc nghiệm liên hệ thực tế.&#10;- Phần tự luận cần có 1 câu hỏi vận dụng cao." className="block w-full px-3 py-2 text-base border-slate-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md shadow-sm flex-grow min-h-[200px]" />
                             </div>
                         </div>
                     </Section>
                )}
            ...
            <div className="flex items-center justify-between pt-6 border-t border-slate-200/80">
                 <div>
                    {step > 1 && (
                        <button type="button" onClick={prevStep} className="px-6 py-3 text-base font-semibold rounded-lg text-slate-700 bg-slate-100 hover:bg-slate-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 transition-all duration-200">
                           Quay lại
                        </button>
                    )}
                </div>
                <div className="flex items-center gap-4">
                    ...
                    {step < totalSteps ? (
                        <button type="button" onClick={nextStep} disabled={isStepInvalid(step)} className="px-8 py-3 bg-indigo-600 text-white text-base font-bold rounded-lg shadow-md hover:bg-indigo-700 transition-all duration-300 disabled:bg-slate-400 disabled:cursor-not-allowed">
                            Tiếp theo
                        </button>
                    ) : (
                         <button type="button" onClick={() => onSubmit(formData)} disabled={isLoading} className="w-full sm:w-auto flex items-center justify-center gap-3 px-8 py-4 bg-indigo-600 text-white text-lg font-bold rounded-lg shadow-lg hover:bg-indigo-700 transition-transform transform hover:scale-105 duration-300 disabled:bg-slate-400 disabled:cursor-not-allowed disabled:scale-100">
                            {isLoading ? (
                                <>
            ...
            <ConfigManagementModal isOpen={isConfigModalOpen} onClose={() => setIsConfigModalOpen(false)} configurations={configurations} onSave={handleSaveConfig} onLoad={handleLoadConfig} onDelete={handleDeleteConfig} />
        </form>
    );
};
